...
#if defined(_FSDK_PDFFORM_FORMFILL_) || defined(_FSDK_ALL_) || defined(_FSDK_PDFFORM_)

CBA_FontMap::CBA_FontMap(FSAnnot* pAnnot, IFX_SystemHandler* pSystemHandler, CPDF_Document* pDocument) :
...
// 	ASSERT(pDocument != NULL);
	m_pAnnotDict = (CPDF_Dictionary*)pAnnot->getDict();
...
CPDF_Dictionary* CBA_FontMap::GetAnnotAPFontList()
{
	ASSERT(m_pAnnotDict != NULL);
	ASSERT(m_pDocument != NULL);
	//to avoid checkbox and radiobutton
	CFX_ByteString bsFT = m_pAnnotDict->GetString("FT");
	if(bsFT.IsEmpty())//if current annot dictionary doesn't have FT entry,we should make sure if its parent has it or not.
	{
		CPDF_Dictionary* parentDict = m_pAnnotDict->GetDict("Parent");
		if(parentDict != NULL)
		{
			bsFT = parentDict->GetString("FT");
		}
	}
	FX_INT32 iFf = m_pAnnotDict->GetInteger("Ff");
	if (0 == iFf)//if current annot dictionary doesn't have Ff entry,we should make sure if its parent has it or not.
	{
		CPDF_Dictionary* parentDict = m_pAnnotDict->GetDict("Parent");
		if (parentDict != NULL)
		{
			iFf = parentDict->GetInteger("Ff");
		}
	}
	if(0 == bsFT.Compare((FX_BSTR)"Btn") && 0 == (iFf & (1<<16)))
	{
		return NULL;
	}
...
void CBA_FontMap::GetAnnotOtherFontAndAddFontData()
{
	ASSERT(m_pDocument != NULL);
	CPDF_Dictionary* pStreamResFontList = GetAnnotAPFontList();
	if (!pStreamResFontList) return;
	FX_POSITION pos = pStreamResFontList->GetStartPos();
	while (pos)
	{
		CPDF_Object* pObj = NULL;
		CFX_ByteString csKey;
		pObj = pStreamResFontList->GetNextElement(pos, csKey);
		...
		if (m_pDefaultFont)
		{
			CPDF_Dictionary* pDefFontDict = m_pDefaultFont->GetFontDict();
			if (pDefFontDict && pElement->GetObjNum() == pDefFontDict->GetObjNum()) continue;
		}

		CPDF_Font* pFont = m_pDocument->LoadFont(pElement);
		if (pFont == NULL) continue;
		if (const CFX_SubstFont* pSubstFont = pFont->GetSubstFont())
			AddFontData(pFont, csKey, pSubstFont->m_Charset);
		else
			AddFontData(pFont, csKey, ANSI_CHARSET);
	}
}
...
void CBA_FontMap::Initial(FX_LPCSTR fontname)
{
	...
	GetAnnotOtherFontAndAddFontData();
...
CPDF_Font* CBA_FontMap::FindFontSameCharset(CFX_ByteString& sFontAlias, FX_INT32 nCharset)
{
	...
	{
		CPDF_Dictionary * pDRDict = GetAnnotDRDict();
		if (!pDRDict) return NULL;

		return FindResFontSameCharset(pDRDict, sFontAlias, "", nCharset);
	}

	if (m_pAnnotDict->GetString("Subtype") == "FreeText")
	{
		CFX_ByteString csDA = m_pAnnotDict->GetString(FX_BSTRC("DA"));
		CPDF_DefaultAppearance da(csDA);
		
		CFX_ByteString csFontNameTag;
		FX_FLOAT fFontSize;
		da.GetFont(csFontNameTag, fFontSize);

		CPDF_Dictionary* pAPDict = m_pAnnotDict->GetDict("AP");
		if (!pAPDict) return NULL;

		CPDF_Dictionary* pAPNDict = pAPDict->GetDict("N");
		if (!pAPNDict) return NULL;

		CPDF_Dictionary* pResDict = pAPNDict->GetDict("Resources");
		if (!pResDict) return NULL;

		return FindResFontSameCharset(pResDict, sFontAlias, csFontNameTag, nCharset);
	}
...
CPDF_Font* CBA_FontMap::FindResFontSameCharset(CPDF_Dictionary* pResDict, CFX_ByteString& sFontAlias, CFX_ByteString sDAFontAlias,
													FX_INT32 nCharset)
{
	if (!pResDict) return NULL;
	...
	while (pos)
	{
		...
		if (!sDAFontAlias.IsEmpty()) 
		{
			if (sDAFontAlias.EqualNoCase(csKey) && pSubst->m_Charset == nCharset)
				bFind = TRUE;
			else
				bFind = FALSE;
		}
		else if (pSubst->m_Charset == nCharset)
		{
			bFind = TRUE;
		}
...
CPDF_Dictionary* CBA_FontMap::GetAnnotDRDict()
{
	if (m_pAnnotDict->GetString("Subtype") == "Widget")
	{
		CPDF_Document* pDocument = GetDocument();
...
void CBA_FontMap::AddedFont(CPDF_Font* pFont, CFX_ByteString& sFontAlias)
{
	AddFontToAnnotDict(pFont, sFontAlias);
}

void CBA_FontMap::AddFontToAnnotDict(CPDF_Font* pFont, CFX_ByteString& sAlias)
{
	if (!pFont)	return;
	ASSERT(m_pDocument != NULL);
	CPDF_Dictionary* pStreamResFontList = GetAnnotAPFontList();
	if (!pStreamResFontList) return;
	CPDF_Dictionary* pNewFontDic = pFont->GetFontDict();
	CPDF_Dictionary* pFontDic = NULL;
	while(pStreamResFontList->KeyExist(sAlias)){
		pFontDic = pStreamResFontList->GetDict(sAlias);
		if(pFontDic && (pFontDic->GetObjNum() == pNewFontDic->GetObjNum()))
			return;
		sAlias += '0';
	}
	pStreamResFontList->SetAtReference(sAlias, m_pDocument, pNewFontDic);
	CPDF_Dictionary * pDRDict = GetAnnotDRDict();
	if (!pDRDict) return;
	CPDF_Dictionary* pFonts = pDRDict->GetDict("Font");
	if (pFonts == NULL) return;
	pFonts->SetAtReference(sAlias, m_pDocument, pNewFontDic);
}
...
CPDF_Font* CBA_FontMap::GetDefaultFont()
{
	return m_pDefaultFont;
}
#endif //defined(_FSDK_PDFFORM_FORMFILL_) || defined(_FSDK_ALL_) || defined(_FSDK_PDFFORM_)
