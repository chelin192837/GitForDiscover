...
CFX_ByteString CPWL_Utils::GetAP_Star(const CPDF_Rect & crBBox)
{
	...
	FX_FLOAT fRadius = (crBBox.top - crBBox.bottom)/(1+(FX_FLOAT)FXSYS_cos(PWL_PI/5.0f));
	...
	{
		px[i] = ptCenter.x + fRadius * (FX_FLOAT)FXSYS_cos(fAngel);
		py[i] = ptCenter.y + fRadius * (FX_FLOAT)FXSYS_sin(fAngel);
...
CFX_ByteString CPWL_Utils::GetAP_HalfCircle(const CPDF_Rect & crBBox,FX_FLOAT fRotate)
{
	...
	csAP << FXSYS_cos(fRotate) << " " << FXSYS_sin(fRotate) << " " << -FXSYS_sin(fRotate) << " " << FXSYS_cos(fRotate) << " " 
...
CFX_ByteString CPWL_Utils::GetRichTextEditAppStream(IFX_Edit* pEdit, const CPDF_Point & ptOffset, const CPVT_WordRange * pRange, 
	FX_BOOL bContinuous)
{
	return IFX_Edit::GetRichEditAppearanceStream(pEdit, ptOffset, pRange, bContinuous, NULL);
}
...
CFX_ByteString CPWL_Utils::GetTextMatrixAppStream(const CPDF_Matrix& mtText)
{
	CFX_ByteTextBuf sTextStream;

	if (mtText.a == 1 && mtText.b == 0 && mtText.c == 0
		&& mtText.d == 1 && mtText.e == 0 && mtText.f == 0)
		return sTextStream.GetByteString();

		sTextStream << mtText.a << " " << mtText.b << " " << mtText.c << " " 
			<< mtText.d << " " << mtText.e << " " << mtText.f << " Tm";
...
{
	FX_FLOAT fLongRadius = (crBBox.top - crBBox.bottom)/(1+(FX_FLOAT)FXSYS_cos(PWL_PI/5.0f));
