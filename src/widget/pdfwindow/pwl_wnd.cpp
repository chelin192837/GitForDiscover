...
static CFX_MapPtrTemplate<FX_INT32, CPWL_Timer*>*	g_pTimeMap = NULL;
...
{	
	...
	{
		if(!g_pTimeMap)
			g_pTimeMap = FX_NEW CFX_MapPtrTemplate<FX_INT32, CPWL_Timer*>();

		g_pTimeMap->SetAt(m_nTimerID, this);
	}
...
void CPWL_Timer::KillPWLTimer()
{
	if (m_nTimerID != 0)
	{
		...
		if(g_pTimeMap)
		{
			g_pTimeMap->RemoveKey(m_nTimerID);
			if(g_pTimeMap->IsEmpty())
			{
				delete g_pTimeMap;
				g_pTimeMap = NULL;
			}
...
void CPWL_Timer::TimerProc(FX_INT32 idEvent)
{
	...
	if (g_pTimeMap && g_pTimeMap->Lookup(idEvent, pTimer))
	{
		if (pTimer)
		{			
...
void CPWL_Wnd::InvalidateRect(CPDF_Rect* pRect)
{
	if (IsValid())
	{
		...
		//Refresh is time wasting if empty.
		if(rcRefresh.IsEmpty()) return;
