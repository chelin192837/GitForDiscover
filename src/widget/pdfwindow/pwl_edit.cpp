...
void CPWL_Edit::GetThisAppearanceStream(CFX_ByteTextBuf & sAppStream)
{
	...
	CFX_ByteString sEditAfter;

	if (!m_pEdit->IsRichText())
		sEditAfter = CPWL_Utils::GetEditAppStream(m_pEdit, ptOffset, &wrTemp, !HasFlag(PES_CHARARRAY), m_pEdit->GetPasswordChar());			
	else
		sEditAfter = CPWL_Utils::GetRichTextEditAppStream(m_pEdit, ptOffset, &wrTemp, !HasFlag(PES_CHARARRAY));

	if (sEditAfter.GetLength() > 0)
	{
		if(m_mtText.f !=0)
			m_mtText.f *= -1;
		CFX_ByteString sTextMatrixAp = CPWL_Utils::GetTextMatrixAppStream(m_mtText);
		if (sTextMatrixAp.IsEmpty())
			sText << "BT\n" << CPWL_Utils::GetColorAppStream(GetTextColor()) << sEditAfter<< "ET\n";
		else
			sText << "BT\n" << CPWL_Utils::GetColorAppStream(GetTextColor()) << CPWL_Utils::GetTextMatrixAppStream(m_mtText) << "\n" << sEditAfter<< "ET\n";
	}
...
FX_BOOL CPWL_Edit::OnRButtonUp(const CPDF_Point & point, FX_DWORD nFlag)
{
	...
	FX_INT32 nCmd = pSH->FXTrackPopupMenu(hPopup,
					 x,
					 y,
					 GetAttachedHWnd());
...
void CPWL_Edit::SetRichText(FX_LPCWSTR xml, FX_LPCWSTR ds /* = NULL */, FX_BOOL bAutoSetAgin /* = TRUE */)
{
	m_pEdit->SetRichTextForXML(xml, ds, bAutoSetAgin);
}
