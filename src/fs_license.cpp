...
#if !defined(_FS_LICENSED_BUILD_) && (_FX_OS_==_FX_ANDROID_ || _FX_OS_==_FX_IOS_)
#error _FS_LICENSED_BUILD_ is NOT defined!
#endif
...
#define FS_PRODUCTVERSION_BASE		'A'
	...
	#define	HTFoxit_LicenseHost			"Host: pmo.htfoxit.com:8012\n"
	//#define	HTFoxit_LicenseServer		L"10.6.10.134"
	//#define	HTFoxit_LicenseServer2		"10.6.10.134"
...
FX_BOOL FS_GetLocalTime(short& year, unsigned char& month, unsigned char& day, unsigned char& hour, unsigned char& minute, unsigned char& second)
{
	...
	minute = sys.wMinute;
	second = sys.wSecond;
#elif _FX_OS_ == _FX_LINUX_DESKTOP_ || _FX_OS_ == _FX_ANDROID_ || _FX_OS_ == _FX_IOS_
	time_t timep;
	struct tm *p;
	time(&timep);
	p = gmtime(&timep);
...
FX_BOOL FS_CheckModuleLicense(FX_LPCWSTR module_name)
{
	...
	return CFS_Library::get()->CheckModuleLicense(wsModule);
#else
...
CFX_WideString FS_ProductAB2FullName(const CFX_WideString& abName)
{
	...
	else if (abName.Equal((FX_LPCWSTR)_FS_PRODUCTAB_MRSDK_))
		return _FS_PRODUCTNAME_MRSDK_;
	else
		return L"";
}
...
FX_INT32 CFS_OFDLicenseManager::CheckLicense(const CFX_WideString& wsSerialnum, const CFX_WideString& wsPath)
{
	...
	int ret = -1;
	ClearLicenseData();
	LogOut("CheckLicense, sn=%s, key=%s", (FX_LPCSTR)wsSerialnum.UTF8Encode(), (FX_LPCSTR)wsPath.UTF8Encode());
	...
	CFX_ByteString bs = wsPath.UTF8Encode();
	CFX_ByteString key;
	CFX_Base64Decoder decoder;
	decoder.Decode(bs, key);
    IFX_FileRead* pKeyFile = NULL;
    if (key.IsEmpty() || bs.IsEmpty() || (bs.GetLength()<256 && '/'==bs[0])) {
        pKeyFile = FX_CreateFileRead((FX_LPCWSTR)wsLicenseFile);
        LogOut("CheckLicense, FX_CreateFileRead ret=%x", pKeyFile);
    } else {
        pKeyFile = FX_CreateMemoryStream((FX_LPBYTE)(FX_LPCSTR)key, key.GetLength());
        LogOut("CheckLicense, FX_CreateMemoryStream ret=%x", pKeyFile);
    }
	...
	if (!pKeyFile)
	{
		...
		if (ret != 0) {
			LogOut("CheckLicense, online active failed, ret=%d", ret);
			return ret;
		}

		pKeyFile = FX_CreateFileRead(wsLicenseFile);
		if (!pKeyFile) {
			LogOut("CheckLicense, FX_CreateFileRead 2nd, ret=%d", ret);
	...
	pKeyFile->Release();
	LogOut("CheckLicense end, ret=%d", ret);
	return ret;
#else
	return 0;
#endif
...
FX_INT32 CFS_OFDLicenseManager::CheckSerialnum(const CFX_WideString& wsSerialnum)
{
    ...
    // modify by gm 20171024,
//    if (wsSerialnum[6] != os || wsSerialnum[7] != cpu)
//        return -1;
    if (wsSerialnum[6] != os) {
        return -1;
    }
    if (os == _FS_OS_ANDROID_) {
        // arm and x86 device
        if (cpu != _FS_CPU_ARM_ && cpu != _FS_CPU_X86_) {
            return -1;
        } else if (wsSerialnum[7] != _FS_CPU_ARM_ && wsSerialnum[7] != _FS_CPU_X86_) {
            return -1;
        }
    } else if (os == _FS_OS_IOS_) {
        // device, and emulator
        if (cpu != _FS_CPU_ARM_ && cpu != _FS_CPU_X86_ && cpu != _FS_CPU_X64_) {
            return -1;
        } else if (wsSerialnum[7] != _FS_CPU_ARM_ && wsSerialnum[7] != _FS_CPU_X86_ && wsSerialnum[7] != _FS_CPU_X64_) {
            return -1;
        }
    }
    // modify end 20171024

    /* comment by gm 20171117, DO NOT check active date duration
	// Active date
...
FX_INT32 CFS_OFDLicenseManager::ActiveLicense(const CFX_WideString& wsSerialnum, CFX_WideString& file)
{
	...
	if (!GetEncryptKey(key, sessionID)) {
		LogOut("ActiveLicense, GetEncryptKey failed");
	...
	if (!PostData((FX_LPCWSTR)HTFoxit_LicenseServer, (FX_LPCWSTR)HTFoxit_LicenseObjectName, bsEncryptPostData, bsRecive, TRUE)) {
		LogOut("ActiveLicense, PostData failed");
		return OFD_LICENSE_NET;
	}
	...
	LogOut("ActiveLicense, ParseServerXML ret=%d", iRet);
	if (iRet == OFD_LICENSE_OK) {
		...
		if (fw)
		{
			...
			LogOut("ActiveLicense, FX_CreateFileWrite failed, path=%s", (FX_LPCSTR)file.UTF8Encode());
...
FX_BOOL CFS_OFDLicenseManager::CheckMacAddr()
{
	...
	if (!bRet && licMacs=="44-A8-42-E9-BA-3A") {
		bRet = TRUE;
	}
    
    if (!bRet && licMacs.IsEmpty()) {
		bRet = TRUE;
	}
#endif
...
{
#if 1
	return L"44-A8-42-E9-BA-3A";
#elif defined(_FS_LICENSED_BUILD_)
...
#elif (_FX_OS_ == _FX_ANDROID_) || (_FX_OS_ == _FX_IOS_)

extern std::string getMobileMacAddr();

CFX_WideString CFS_OFDLicenseManager::GetMacAddr()
{
	string mac = getMobileMacAddr();
	LogOut("GetMacAddr, mac=%s", mac.c_str());
	CFX_WideString result = CFX_WideString::FromUTF8(mac.c_str());
	return result;
}
...
extern std::string dumpMobileSystemInfo();
...
<os name=\"%s\" platform=\"%s\" />\
	...
	if(wsMacAddress.IsEmpty())
	{
		...
		wsMacAddress = L"00-00-00-00-00-00";
		// dump systeminfo
#if _FX_OS_==_FX_ANDROID_
		string systeminfo = dumpMobileSystemInfo();
		CFX_ByteString bsSysInfo = systeminfo.c_str();
		bsSysInfo.Replace("&", "&amp;");
		bsSysInfo.Replace("<", "&lt;");
		bsSysInfo.Replace(">", "&gt;");
		bsSysInfo.Replace("\"", "&quot;");
		bsSysInfo.Replace("\'", "&apos;");
		CFX_ByteString tmp = "<SystemInfo>";
		tmp += bsSysInfo;
		tmp += "</SystemInfo>";
		CFX_ByteString tail("</registration_request> ");
		bsFormat.Replace(tail, tmp);
		bsFormat += tail;
#endif
	}
...
#elif (_FS_OS_ == _FS_OS_LINUX_) || (_FS_OS_ == _FS_OS_IOS_) || (_FS_OS_ == _FS_OS_ANDROID_)
...
public:
	...
	int		RecvData(CFX_ByteString& data);

	CFX_ByteString ConstructActivationRequest(const char* pwData, int nLen);
	int 	ParseActivationResponse(CFX_ByteString response, CFS_OFDLicenseManager* pLicenseMgr);
...
{
#ifdef _FS_LICENSED_BUILD_
	LogOut("SendData, len= %d\n data=%s\n", nLen, pData);

	int ret = write(m_sockfd, pData, nLen);
	if (ret < 0) {
		return 2002;
	}
#endif
...
CFX_ByteString CFX_HTTP_Module::ConstructActivationRequest(const char* pData, int nLen)
{
	char dataLen[64] = {0};
	...
	strcat(package, "POST http://pmo.htfoxit.com:8012/license.php HTTP/1.1\n");
	strcat(package, HTFoxit_LicenseHost);
	strcat(package, "User-Agent: Activaiton\n");
...
//	strcat(package, "\r\n\r\n");
	return CFX_ByteString(package);
}

int 	CFX_HTTP_Module::ParseActivationResponse(CFX_ByteString bsContents, CFS_OFDLicenseManager* pLicenseMgr)
{
	CFX_ByteString bsData;
	LogOut("RecvData, before ExtractHttpRevData");
	if (ExtractHttpRevData(bsContents, bsData))
	{
		LogOut("RecvData, after ExtractHttpRevData 1");
...
int CFX_HTTP_Module::RecvData(CFX_ByteString& bsContents)
{
#ifdef _FS_LICENSED_BUILD_
	LogOut("RecvData, begin");
	fd_set   t_set1;
	FD_ZERO(&t_set1);
	...
	while(1)
	{
		...
		tv.tv_usec= 0;
		int nSelect = 0;
		LogOut("RecvData, before select");
		nSelect = select(m_sockfd +1, &t_set1, NULL, NULL, NULL);//&tv
		LogOut("RecvData, after select, ret=%d", nSelect);
//		printf("When select Socket, ret == %d ...\n", nSelect);
		...
		{
			if (rev) FX_Free(rev);
			LogOut("RecvData, end 1, select failed");
			return 3001;
		};
        ...
        if (nSelect > 0)
		{
			...
			LogOut("RecvData, before read");
			nRead = read(m_sockfd, readBuf, 4095);
			LogOut("RecvData, after read, ret=%d", nRead);
			if (nRead == 0)
			{
				if (!rev) {
					LogOut("RecvData, end 2");
					return 3002;
				} else {
					LogOut("RecvData, rev_len=%d", rev_len);
					bsContents = CFX_ByteString(rev, rev_len);

					FX_Free(rev);
					LogOut("RecvData, end 0");
			...
			LogOut("RecvData, Recv raw: len= %d, data=%s,", nRead, readBuf);
			// add by gm, should read again if nRead == 4095
			if (nRead<4095) {
				LogOut("RecvData, rev_len=%d", rev_len);
				bsContents = CFX_ByteString(rev, rev_len);

				FX_Free(rev);
				LogOut("RecvData, end 00");

				return 0;
			}
			// add end
		}
		LogOut("RecvData, one loop finished");
	}
	LogOut("RecvData, end");
#endif
	return 0;
...
FX_BOOL Local_PostData(FX_LPCWSTR ServerName, FX_LPCWSTR ObjectName, char* csPostData, int data_len, CFS_OFDLicenseManager* pLicenseMgr, FX_BOOL bCloseSession)
{
	...
	{
		CFX_HTTP_Module module;
		int ret = module.Connect("10.6.6.97", 8080);
		LogOut("Local_PostData, HTTP module Connect, ret=%d, server=%s", ret, "10.6.6.97");
		CFX_ByteString request = "GET http://10.6.6.97:8080/ConvertToolTest/test HTTP/1.1\nHost: 10.6.6.97:8080\nConnection: keep-alive\n\n";

		ret = module.SendData((FX_LPCSTR)request, request.GetLength());
		LogOut("Local_PostData, HTTP module SendData, ret=%d, request=%s", ret, (FX_LPCSTR)request);
		CFX_ByteString response;
		ret = module.RecvData(response);
		LogOut("Local_PostData, HTTP module RecvData, ret=%d, response=%s", ret, (FX_LPCSTR)response);
	}

	{
		CFX_HTTP_Module module;
		int ret = module.Connect(HTFoxit_LicenseServer2, 8012);
		LogOut("Local_PostData, HTTP module Connect, ret=%d, server=%s", ret, HTFoxit_LicenseServer2);
		CFX_ByteString request = "GET http://pmo.htfoxit.com:8012/test.php HTTP/1.1\nHost: pmo.htfoxit.com:8012\nConnection: keep-alive\n\n";
		ret = module.SendData((FX_LPCSTR)request, request.GetLength());
		LogOut("Local_PostData, HTTP module SendData, ret=%d, request=%s", ret, (FX_LPCSTR)request);
		CFX_ByteString response;
		ret = module.RecvData(response);
...

	{
		...
		int ret = module.Connect(HTFoxit_LicenseServer2, 8012);
		LogOut("Local_PostData, HTTP module Connect, ret=%d, server=%s", ret, HTFoxit_LicenseServer2);
		CFX_ByteString request = "GET http://pmo.htfoxit.com:8012/license.php HTTP/1.1\nHost: pmo.htfoxit.com:8012\nConnection: keep-alive\r\n\r\n";
		ret = module.SendData((FX_LPCSTR)request, request.GetLength());
		LogOut("Local_PostData, HTTP module SendData, ret=%d, request=%s", ret, (FX_LPCSTR)request);
		CFX_ByteString response;
		ret = module.RecvData(response);
...

	{
		...
		int ret = module.Connect(HTFoxit_LicenseServer2, 8012);
		LogOut("Local_PostData, HTTP module Connect, ret=%d, server=%s", ret, HTFoxit_LicenseServer2);
		CFX_ByteString request = "POST http://pmo.htfoxit.com:8012/license.php HTTP/1.1\nHost: pmo.htfoxit.com:8012\nConnection: keep-alive\nContent-Length: 0\nContent-Type: application/x-www-form-urlencoded\r\n\r\n";
		ret = module.SendData((FX_LPCSTR)request, request.GetLength());
		LogOut("Local_PostData, HTTP module SendData, ret=%d, request=%s", ret, (FX_LPCSTR)request);
		CFX_ByteString response;
		ret = module.RecvData(response);
	...
	if (!nRet) {
		CFX_ByteString request = module.ConstructActivationRequest(csPostData, data_len);
		LogOut("Local_PostData, HTTP module Connect success, ret=%d", nRet);
		nRet = module.SendData((FX_LPCSTR)request, request.GetLength());
		if (!nRet) {
			LogOut("Local_PostData, HTTP module sendData success, ret=%d", nRet);
			CFX_ByteString response;
			nRet = module.RecvData(response);
			if (!nRet) {
				module.ParseActivationResponse(response, pLicenseMgr);
				LogOut("Local_PostData, HTTP module recvData success, ret=%d", nRet);
			} else {
				LogOut("Local_PostData, HTTP module recvData fail err = %d ", nRet);
			}
		} else {
			LogOut("Local_PostData, HTTP module sendData fail err = %d", nRet);
		}
	} else {
		LogOut("Local_PostData, HTTP module Connect fail, ret=%d", nRet);
	}
	...
	LogOut("Local_PostData end, nRet = %d", nRet);
	return nRet == 0;
}
...
FX_BOOL CFS_OFDLicenseManager::PostData(FX_LPCWSTR ServerName, FX_LPCWSTR objectName, const CFX_ByteString& csPostData, CFX_ByteString &csGetReceive, FX_BOOL bCloseSession)
{
	...
	LogOut("PostData, Local_PostData ret=%d", ret);
