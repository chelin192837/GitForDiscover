...
enum FS_ERRORCODE {
    ...
    /** @brief    Not parsed error: content has not been parsed yet. Usually, this represents PDF page has not been parsed yet. */
    e_errNotParsed = 12,
    /** @brief    Network error: fail to access license server. */
    e_errNetwork = 13,
    /** @brief    License error: exceeded number of authorized devices. */
    e_errLicenseExceed = 14,
    /** @brief    License error: license has expired. */
    e_errLicenseExpired = 15,
    /** @brief    License error: fetch device id failed, or device hardware changed after authorization. */
    e_errDeviceChanged = 16,
    /** @brief    License error: lincense version is lower than SDK version. */
    e_errLicenseVersion = 17
};
 ...
 *            {@link FS_ERRORCODE::e_errInternet} means fail to access the Internet.
 */
...
/**
 * @brief    Set image file to current stamp annotation.
 *
 * @param[in]    imagePath    A image file path to be set to current stamp.
 */
...
/**
* @brief    Convert a PDF to OFD with a memory buffer.
*
* @param[in]    buffer        A memory buffer, containing the serialized document.
*                            The PDF document data should be fully loaded in the memory buffer.<br>
*                            The memory buffer ownership is not transferred to the PDF document object,
...
/**
* @brief   Convert a OFD to PDF with a memory buffer.
*
* @param[in]    buffer        A memory buffer, containing the serialized document.
*                            The PDF document data should be fully loaded in the memory buffer.<br>
*                            The memory buffer ownership is not transferred to the PDF document object,
...
/**
* @brief    Convert a PDF to OFD form Path
*
* @param[in]    buffer        A memory buffer, containing the serialized document.
*                            The PDF document data should be fully loaded in the memory buffer.<br>
*                            The memory buffer ownership is not transferred to the PDF document object,
...
+(NSArray<FSSeal*>*)GetSealList:(NSString*)password;

-(NSString*)GetSealWithPassword:(NSString*)password SealID:(NSString*)sealID;

-(NSString*)GetCertWithPassword:(NSString*)password SealID:(NSString*)sealID;

-(FSPDFDoc*)VerifySealSignAll:(NSString*)password;

-(NSString*)VerifySealSign:(NSString*)password SealID:(NSString*)sealID IsVerify:(BOOL)isVerify;
...
enum FS_SIGNATUREKEYNAME {
    ...
    /** @brief    OFD Signature key name: provider. */
    e_signatureKeyNameProvider = 10,
    /** @brief    OFD Signature key name: company. */
    e_signatureKeyNameCompany = 11,
    /** @brief    OFD Signature key name: version. */
    e_signatureKeyNameVersion = 12,
    /** @brief    OFD Signature key name: seal id. */
    e_signatureKeyNameSealId = 13,
    /** @brief    OFD Signature key name: seal name. */
    e_signatureKeyNameSealName = 14
};
...
enum FS_DIGEST_ALGORITHM {
    ...
    e_digestSHA512 = 3,
    /** @brief    Signature digest algorithm: sm3 algorithm. */
    e_digestSM3 = 10
};
    
    
/**
 * @brief    Class to represent a callback object as signature handler.
 *
 * @details    All the functions int this class are used as callback functions and should be implemented by user.
 */
...
/**
 * @brief    A callback function used to get cert data from cert file or smart hardware.
 *
 * @details    Host application provides this callback method for custom cert parsing.
 ...
 * @param[in]    clientData        Pointer to a user-defined object, which is passed by user from function {@link FSSignature::startSign} or {@link FSSignature::startVerify}.
 *
 * @return    x509 cert data buffer
 *            If there is any error, please return nil.
 */
...
/**
 * @brief    Set a image file for the signature appearance.
 *
 * @details    This function is recommended to be used before calling function {@link FSSignature::startSign:certPath:certPassword:digestAlgorithm:pause:clientData:} for an unsigned signature.
 *            If customized appearance content has been set by function {@link FSSignature::setAppearanceContent:}, the image will not be used in signed appearance.
 *
 * @param[in]    imagePath        A image file path to be set to the appearance. It should be valid.
 */
-(void)setImagePath: (NSString*)imagePath;
...
/**
 * @brief    Set external signature handler
 *
 * @details    This function can only be used for an unsigned signature, before calling function {@link FSSignature::startSign:certPath:certPassword:digestAlgorithm:pause:clientData:} for the unsigned signature.
 *            Once external signature handler is set, it will be used to get cert data and calc sign value
 *
 * @param[in]    siangtureHandler        External signature handler, Provide x.509 cert data and sign function.
 */
...
@interface FSSealImage : NSObject
{
/** @brief SWIG proxy related property, it's deprecated to use it. */
void *swigCPtr;
/** @brief SWIG proxy related property, it's deprecated to use it. */
...
@property (nonatomic,strong) NSString* sealID;
